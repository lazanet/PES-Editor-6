...
DropTargetListener, DragSourceListener, DragGestureListener
{
	...
	public TransferPanel(PlayerDialog pd, OptionFile opf, FormationDialog td)
	{
		...
		autoRep.setToolTipText("Gaps made in a team's first 11 will be automatically filled with the most appropriate sub");
		...
		selectorL.squadList.setToolTipText("Double click to edit player, right click to edit formation");
		selectorR.squadList.setToolTipText("Double click to edit player, right click to edit formation");
	...
	private int clubRelease(int p, boolean rel)
	{
		...
		do {
			...
			if (index == p)
			{
				...
				if (rel)
				{ of.data[a] = 0; of.data[a + 1] = 0; of.data[getNumAdr(a)] = -1;
					if (sp >= 22)
					{
						Squads.tidy( of , sqi);
					}
		...
		Player targetPlayer)
	{
		...
		if (safe)
		{
			...
			if ((squadL >= 64) || ((squadR >= 64) || ((squadL > 72) && (squadL < 213))))
			{
	...
	private void transferFL(int index)
	{
		...
		} of.data[adr] = of.toByte(index & 0x000000FF); of.data[adr + 1] = of.toByte((index & 0x0000FF00) >>> 8);
	...
	private void transferLR(Player player)
	{
		...
		if (index != 0)
		{
			...
			of.data[adrR] = of.toByte(index & 0x000000FF); of.data[adrR + 1] = of.toByte((index & 0x0000FF00) >>> 8);
	...
	private void transferRL(Player player)
	{
		...
		if (index != 0)
		{
			...
			of.data[adrL] = of.toByte(index & 0x000000FF); of.data[adrL + 1] = of.toByte((index & 0x0000FF00) >>> 8);
		...
		JList sourceList, JList targetList)
	{
		...
		of.data[adrS] = of.toByte(indexT & 0x000000FF); of.data[adrS + 1] = of.toByte((indexT & 0x0000FF00) >>> 8); of.data[adrT] = of.toByte(indexS & 0x000000FF); of.data[adrT + 1] = of.toByte((indexS & 0x0000FF00) >>> 8);

		if (tiS == tiT)
		{
			byte t = of.data[getNumAdr(adrT)]; of.data[getNumAdr(adrT)] = of.data[getNumAdr(adrS)]; of.data[getNumAdr(adrS)] = t;
		}
		...
		if (indexS == 0 || indexT == 0)
		{
			...
			else
			{
				if (autoRep.isSelected())
				{
					Squads.tidy11(of,tiS, sourceIndex,((SelectByTeam)(sourceList.getParent())).posList.posNum[sourceIndex]);
				}
			}
	...
	{
		int adr = player.adr; of.data[adr] = 0; of.data[adr + 1] = 0; of.data[getNumAdr(adr)] = -1;
		if (si > 10)
		{
			Squads.tidy( of , selectorL.teamBox.getSelectedIndex());
		}
	...
	private void tranRelR(Player player, int si)
	{
		...
		else
		{
			if (autoRep.isSelected())
			{
				Squads.tidy11(of, selectorR.teamBox.getSelectedIndex(), si,selectorR.posList.posNum[si]);
			}
		}
		refreshLists();
	}
