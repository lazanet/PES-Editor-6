...
public class EmblemPanel extends JPanel implements MouseListener
{
	...
	public EmblemPanel(OptionFile opt, EmblemImportDialog fid, TeamPanel tp)
	{
		...
		flagButton = new JButton[Emblems.totalB];
		flagPanel = new JPanel(new GridLayout(6, 10));

		PESUtils.javaUI();

		for (int l = 0; l < Emblems.totalB; l++)
		{
				...
				public void actionPerformed(ActionEvent b)
				{
					...
					if (flagImpDia.of2Open && n == 3)
					{
						...
						else
						{
							...
							if (replacement != -1)
							{
								replacement = replacement - Emblems.total;
		...
		add2Button.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent t)
			{
				...
				if (emblem != -1)
				{
					if (emblem > Emblems.total - 1)
					{
						emblem = emblem - Emblems.total;
	...
	public void refresh()
	{
		...
		{
			flagButton[Emblems.totalB - 1 - i].setIcon(new ImageIcon(Emblems
				.get128( of , i, !trans, true)));
			flagButton[Emblems.totalB - 1 - i].setVisible(true);
		}

		for (int i = Emblems.count16( of ); i < Emblems.totalB - Emblems.count128( of ); i++)
		{
